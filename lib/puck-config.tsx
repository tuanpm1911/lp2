...
export const puckConfig: Config<Components,RootProps> = {
  root:{
    ...
    render:({children,brandColor,fontStyle,radius})=>{
      const style={
  ...
  components:{
    HeroSplit:{
      ...
      render:({variant,eyebrow,title,description,ctaLabel,ctaHref,imageUrl,imageAlt})=>{
        const centered=variant==='centered';
        const dark=variant==='dark';
        const sectionStyle:React.CSSProperties={padding:'76px 0',background:dark?'#07111f':'linear-gradient(135deg,#f8fafc,#fff)',color:dark?'#fff':'#0f172a',borderTop:'4px solid var(--lp-brand)'};
    ...
    ProblemSolution:{
      ...
      render:({title,problem,solution})=><section style={{padding:'72px 0'}}><div style={shell}><h2 style={{fontSize:36,marginBottom:28}}>{title}</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:18}}><article style={{padding:28,border:'1px solid #e2e8f0',borderRadius:'var(--lp-radius)'}}><strong>Vấn đề</strong><p style={{lineHeight:1.7,color:'#475569'}}>{problem}</p></article><article style={{padding:28,border:'1px solid var(--lp-brand)',background:'#f8fafc',borderRadius:'var(--lp-radius)'}}><strong>Giải pháp</strong><p style={{lineHeight:1.7,color:'#334155'}}>{solution}</p></article></div></div></section>
    },
    BenefitCards:{
      fields:{title:{type:'text'},items:{type:'textarea'}},defaultProps:{title:'Lợi ích nổi bật',items:'Nhanh hơn\nDễ sử dụng\nTối ưu chi phí'},
      render:({title,items})=><section style={{padding:'72px 0',background:'#f8fafc'}}><div style={shell}><h2 style={{fontSize:36,marginBottom:28}}>{title}</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16}}>{lines(items).map((x,i)=><div key={i} style={{padding:24,background:'#fff',border:'1px solid #e2e8f0',borderTop:'3px solid var(--lp-brand)',borderRadius:'var(--lp-radius)',fontWeight:700}}>{x}</div>)}</div></div></section>
    },
    HowItWorks:{fields:{title:{type:'text'},steps:{type:'textarea'}},defaultProps:{title:'Cách hoạt động',steps:'Bước 1\nBước 2\nBước 3'},render:({title,steps})=><section style={{padding:'72px 0'}}><div style={shell}><h2 style={{fontSize:36}}>{title}</h2>{lines(steps).map((x,i)=><div key={i} style={{display:'flex',gap:16,padding:'18px 0',borderBottom:'1px solid #e2e8f0'}}><b style={{color:'var(--lp-brand)'}}>{String(i+1).padStart(2,'0')}</b><span>{x}</span></div>)}</div></section>},
...
export function pageSpecToPuck(spec:PageSpec,brief?:MarketingBrief):Data{
  ...
  return {content,root:{props:{title:spec.seo.title||spec.page.name,brandColor:spec.design.brandColor||brief?.brandColor||'#2563eb',fontStyle:spec.design.fontStyle||'modern',radius:spec.design.radius||'14px'}}} as Data;
}
